...
function createBubble(fromBottom = false) {
  return {
    ...
    y: fromBottom
      ? oceanCanvas.height + 10
      : Math.random() * oceanCanvas.height,
    r: Math.random() * 4 + 1,
...
function drawOcean() {
  ...
  if (bubblesEnabled) {
    bubbles.forEach((b, i) => {
      ...
      octx.lineWidth = 0.8;
      octx.stroke();
      const hgl = octx.createRadialGradient(
        b.x - b.r * 0.3,
        b.y - b.r * 0.3,
        0,
        b.x,
...
function drawFishTank() {
  ...
  FISH.forEach((f) => {
    ...
    if (f.vx < 0) {
      fctx.scale(-1, 1);
      fctx.fillText(f.emoji, -f.x - f.size, f.y + f.size);
    } else {
      fctx.fillText(f.emoji, f.x, f.y + f.size);
    }
    fctx.restore();
  });
  ...
  window.addEventListener(
    ...
    () => {
      nav.style.background =
        window.scrollY > 60 ? "rgba(7,30,51,0.95)" : "rgba(7,30,51,0.75)";
    },
    { passive: true },
  );
}
...
function setupFadeIn() {
  ...
  document
    .querySelectorAll(
      "section, .project-card, .skill-card, .achievement-card, .lang-card, .timeline-item",
    )
    .forEach((el) => {
      el.style.opacity = "0";
